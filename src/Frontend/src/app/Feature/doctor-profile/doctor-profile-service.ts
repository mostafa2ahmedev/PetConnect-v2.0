...
export class DoctorProfileService {
  ...
  getTimeSlotsForDoctor(doctorId: string): Observable<{data:DataTimeSlotsDto[],statusCode:number}> {
    return this.httpClient.get<{data:DataTimeSlotsDto[],statusCode:number}>(`${this.server}/api/TimeSlots/${doctorId}`);
  }
  getTimeSlotsForBookingWithStatus(doctorId:string): Observable<ApiResponse<TimeSlotsWithStatusDTO[]>>{
    return this.httpClient.get<ApiResponse<TimeSlotsWithStatusDTO[]>>(`${this.server}/api/TimeSlots/view/${doctorId}`);
