...
import { DataTimeSlotsDto } from '../../../Doctor/doctor-profile/data-time-slot-dto';
...
export class ShowAllDoctorTimeslotsService {
  ...
  ShowAllTimeSlots(): Observable<{
    ...
    return this.httpClient.get<{
      statusCode: number;
      data: DataTimeSlotsDto[];
    }>(`${this.server}/api/TimeSlots/all/${userId}`);
  }

  changeStatusToActive(
    slot: ShowAllDoctorTimeslotsDto
  ): Observable<ApiResponse<string>> {
    return this.httpClient.put<ApiResponse<string>>(
      `${this.server}/api/TimeSlots/Active`,
      slot,
      {
