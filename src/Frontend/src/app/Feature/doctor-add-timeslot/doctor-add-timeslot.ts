...
@Component({
  ...
  imports: [CommonModule, FormsModule,MatFormFieldModule, MatInputModule,MatDatepickerModule, MatButtonModule,MatCheckboxModule,MatNativeDateModule,ReactiveFormsModule],
...
export class DoctorAddTimeslot {
  ...
  submitSchedule() {
    ...
    const { scheduleDate, startTime,endTime, maxCapacity, isActive } = this.scheduleForm.value;

    // Combine date and time into ISO format
    const combinedDateStartTime = new Date(`${this.formatDate(scheduleDate)}T${startTime}:00Z`).toISOString();
    const combinedDateEndTime = new Date(`${this.formatDate(scheduleDate)}T${endTime}:00Z`).toISOString();
