...
export class DoctorCustomerAppointment implements OnInit {
  ...
  confirmBooking(){
  if (this.appointmentForm.valid && this.slot.bookedCount< this.slot.maxCapacity) {

  this.doctorCustomerAppService.postPetAppointmentForDoctor(this.appointmentForm.value).subscribe({
    next: resp=>{
      // console.log(resp);
      this.alertService.confirm("successfully created appointment");
      this.router.navigateByUrl(`/doctors/${this.doctorId}`)
       this.doctorCustomerAppService.increaseBookedCountByOne(this.slot,this.doctorObj.id).subscribe({
        next:resp=>{
    }
  })
    },
