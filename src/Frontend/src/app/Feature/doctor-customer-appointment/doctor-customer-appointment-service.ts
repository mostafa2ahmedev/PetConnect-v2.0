...
export class DoctorCustomerAppointmentService {
  ...
  getPetsForCustomer():Observable<{statusCode:number,data:PetsForCustomerDto[]}>{
   const user =this.accountService.jwtTokenDecoder();
   const headers = new HttpHeaders({ 'Accept': 'application/json' });
   return this.httpClient.get<{statusCode:number,data:PetsForCustomerDto[]}>(`${this.server}/api/Pet/Customer/${user.userId}`, { headers });
  ...
  getAppointmentsForCurrentDoctor():Observable<DoctorCustomerAppointmentView[]>{
   const user =this.accountService.jwtTokenDecoder();
    return this.httpClient.get<DoctorCustomerAppointmentView[]>(`${this.server}/api/Appointment/Doctor/${user.userId}`);
  }

  getAppointmentsForDoctorProfileView():Observable<DoctorProfileAppointmentView[]>{
      const user =this.accountService.jwtTokenDecoder();
    return this.httpClient.get<DoctorProfileAppointmentView[]>(`${this.server}/api/Appointment/DoctorProfile/${user.userId}`);
  }
}
